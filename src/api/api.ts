...
// api 호출 함수
export const fetchRankingProducts = async (params: GetRankingProductsRequest): Promise<GetRankingProductsResponse> => {
    const response = await axiosInstance.get<GetRankingProductsResponse>('/api/v1/ranking/products', { params });
...
export const fetchThemeProducts = async (params: GetThemeProductsRequest): Promise<GetThemeProductsResponse> => {
    const { themeKey, ...queryParams } = params;
    const response = await axiosInstance.get<GetThemeProductsResponse>(`/api/v1/themes/${themeKey}/products`, { params: queryParams });
    return response.data;
};

export const fetchProductDetail = async (params: GetProductDetailRequest): Promise<GetProductDetailResponse> => {
    const { productId } = params;
    const response = await axiosInstance.get<GetProductDetailResponse>(`/api/v1/products/${productId}/detail`);
    return response.data;
};

export const fetchProductOptions = async (params: GetProductOptionsRequest): Promise<GetProductOptionsResponse> => {
    const { productId } = params;
    const response = await axiosInstance.get<GetProductOptionsResponse>(`/api/v1/products/${productId}/options`);
    return response.data;
};

export const fetchMessageCardTemplates = async (): Promise<GetMessageCardTemplatesResponse> => {
    const response = await axiosInstance.get<GetMessageCardTemplatesResponse>('/api/v1/message-card/templates');
    return response.data;
};
...
export const fetchWishProducts = async (params: GetWishProductsRequest): Promise<GetWishProductsResponse> => {
    const response = await axiosInstance.get<GetWishProductsResponse>('/api/v1/my-account/wish/products', { params });
    return response.data;
};

export const putMyAccountPoint = async (data: PutMyAccountPointRequest): Promise<PutMyAccountPointResponse> => {
    const response = await axiosInstance.put<PutMyAccountPointResponse>('/api/v1/my-account/point', data);
    return response.data;
};
